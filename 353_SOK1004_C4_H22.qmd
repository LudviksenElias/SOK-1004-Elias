...
***Alle unike verdier blir telt ved*** **binwidth = 0.1 *fordi i variabelen mpg er det mange forskjellige tall med desimaler som gjør at for hver unike mål i mpg får sin egen graf i histogramet.***
...
***Det som kjennetegner en god binwidth verdi er når den viser variert høyde på de ulike aksene, altså at den ikke blir flat men går opp og ned. I tilegg kjennetegner en god binwidth når aksene samsvarer med posisjonen til x-aksen, altså at den treffer bent på tallet som i figuren ovenfor der den er i 1:1 ratio.***
...
df <- df %>% 
  ...
  # Endrer til int istede for chr, og deler verdi på 1000 for å vise verdien som miliarder
...
df %>%
  ...
  # Filtre ut sysselsatte under 20 ansatte
  filter(gruppe >= 20) %>%
  
  # Velger år mellom 2015-2020
  filter(between(år, 2015, 2020)) %>%
  
  # Plotter et histogram der y er verdi og fargene inni boksene er antall sysselsatte
  ggplot(aes(y = verdi, fill = gruppe)) +
  
  # Endrer bindwidth og setter kantfargen til svart for enklere forståelse av figur
  geom_histogram(binwidth = 2.5, color = "black") + 
  
  # Legger til flere figurer for å vise fordelingen for alle næringsgruppene
  facet_grid(næring ~ .) +
  
  # Gir navn
  labs(y = "Fordeling av milliarder Kr",
       x = "Antall",
       fill = "Gruppe",
       title = "Fordelingen av FoU-kostnader i milliarder av Kr") + 
...
df %>%
  ...
  # Plotter et histogram der y er verdi og fargene inni boksene er de forskjellige variablene i Var
  ggplot(aes(y = verdi, fill = var)) +
